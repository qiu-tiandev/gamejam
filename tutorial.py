...
class TutorialManager:
    ...
    def _start_paragraph(self, index):
        ...
        self.wrapped_lines = self._wrap_text(
            self.paragraphs[index], font, self.font_size, max_text_width
        )
    ...
    def _show_full_paragraph(self):
        ...
        max_text_width = w - 2 * self.text_x
        self.wrapped_lines = self._wrap_text(
            self.paragraphs[self.current_paragraph],
            font,
            self.font_size,
            max_text_width,
        )
        ...
        self.renderer.createText(
            "TEMPTYPINGANIMATION", wrapped_text, "Arial", 18, text_color, True
        )
    ...
    def render(self):
        ...
        if self.current_paragraph == 2 and self.craft_manager:  # Crafting paragraph
            ...
            self.current_paragraph == 9 and self.player
        ):  # Health and hunger bars paragraph
        ...
        if self.typing_anim:
            ...
            if self.animation_complete and hasattr(self, "wrapped_lines"):
                ...
                self.renderer.render(
                    ["TEMPTYPINGANIMATION"], [(self.text_x, self.text_y)]
                )
