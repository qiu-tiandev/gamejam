...
class InventoryManager:
    ...
    def renderInventory(self):
        ...
        for idx in range(1, self.num_slots + 1):
            ...
            hotbar_id = (
                self.hotbar_selected_id
                if idx == self.currentSlot
                else self.hotbar_unselected_id
            )
            hotbar_tex = self.renderer.getTexture(hotbar_id)
            if hotbar_tex:
                scaled_hotbar = pygame.transform.scale(
                    hotbar_tex, (slot_size, slot_size)
                )
            ...
            if idx <= len(items_list):
                ...
                if surf is None:
                    base_id = (
                        icon_id
                        if self.renderer.getTexture(icon_id)
                        else "placeholder-item"
                    )
                    surf = self.renderer.ResizeTexture(
                        base_id, [icon_size, icon_size], False, save=True
                    )
                    if surf:
                ...
                text_color = (
                    util.getContrastColor(self.world.themeColor)
                    if self.world
                    else (255, 255, 255)
                )
                tex_id = f"inv_amt_slot_{idx}"
                self.renderer.createText(
                    tex_id, str(amount), "arial", 18, text_color, True, silence=True
                )
    ...
    def dropCurrent(self, itemManager, player, amount: int = 1):
        ...
        if self.currentSlot - 1 < len(items_list):
            ...
            itemManager.addItemEntities(
                item_id,
                amt,
                (player.x + offset, util.getScreenDimensions()[1] - self.slotSize),
            )
    ...
    def renderHeldItem(self, player):
        ...
        if surf is None:
            base_id = (
                icon_id if self.renderer.getTexture(icon_id) else "placeholder-item"
            )
            base_surf = self.renderer.ResizeTexture(
                base_id, [held_size, held_size], False, save=False
            )
            if base_surf:
                if self.lastDir == -1:
        ...
        if surf:
            if self.lastDir == -1:
                ...
                item_x = (
                    util.getScreenDimensions()[0] // 2 + 10 - held_size // 2 + 5
                )  # 5px is offset (so items doesnt look so squished)
            item_y = player.y
        ...
        if item_id == util.getItemID("laser gun") and amount > 0:
            ...
            self.renderer.createAndRenderText(
                ...
                "arial",
                18,
                (0, 255, 0) if player.lasergun_energy > 30 else (255, 0, 0),
                (w // 2 - 60, 35),
                cache=False,
                silence=True,
            )
...
class CraftingManager:
    ...
    def render(self):
        ...
        if self.gui_scaled:
            ...
            pygame.draw.line(
                ...
                self.world.themeColor,
                (line_x, round(self.gui_y * 1.4)),
                (line_x, self.gui_y + self.gui_h),
                4,
            )
            ...
            num_columns = (
                num_items + max_per_column - 1
            ) // max_per_column  # Ceiling division

            for idx, item_name in enumerate(self.craftables):
                column = idx // max_per_column
                row = idx % max_per_column
                slot_pos = (
                    self.list_x + column * (self.slot_size + 10),
                    self.list_y + row * self.item_spacing,
                )

                hotbar_tex = self.renderer.getTexture(self.hotbar_unselected_id)
                if hotbar_tex:
                    scaled_hotbar = pygame.transform.scale(
                        hotbar_tex, (self.slot_size, self.slot_size)
                    )
                ...
                pygame.draw.rect(
                    self.renderer.screen,
                    outline_color,
                    (slot_pos[0], slot_pos[1], self.slot_size, self.slot_size),
                    2,
                )
                ...
                if surf is None:
                    ...
                    surf = self.renderer.ResizeTexture(
                        base_id, [icon_size, icon_size], False, save=False
                    )
                    if surf:
                ...
                if (
                    slot_pos[0] <= mouse_x <= slot_pos[0] + self.slot_size
                    and slot_pos[1] <= mouse_y <= slot_pos[1] + self.slot_size
                ):
            ...
            if self.selected_recipe >= 0 and self.selected_recipe < len(
                self.craftables
            ):
    ...
    def render_recipe_details(self, recipe_idx):
        ...
        row_spacing = (
            icon_size + text_offset_y + icon_size // 2 + 10
        )  # Space between rows
        ...
        for row in range(rows):
            ...
            if use_two_rows:
                row_width = len(row_ingredients) * item_spacing - (
                    item_spacing - icon_size
                )
                start_x = right_section_start + (right_section_width - row_width) // 2

            for ingredient_idx, (material_name, required_amount) in enumerate(
                row_ingredients
            ):
                material_id = util.getItemID(material_name)
                current_amount = self.inventory.items.get(material_id, 0)
                ...
                self.renderer.createAndRenderText(
                    f"recipe_ingredient_{row}_{ingredient_idx}",
                    material_name,
                    "arial",
                    font_size,
                    text_color,
                    (name_x, y_pos + text_offset_y),
                    cache=False,
                    silence=True,
                )
                ...
                self.renderer.createAndRenderText(
                    f"recipe_ingredient_amt_{row}_{ingredient_idx}",
                    amnt_vs_req,
                    "arial",
                    font_size,
                    text_color,
                    (text_x, y_pos + text_offset_y + icon_size // 2),
                    cache=False,
                    silence=True,
                )
        ...
        if recipe_description:
            ...
            description_x = (
                right_section_start + (right_section_width - text_width) // 2
            )
            self.renderer.createAndRenderText(
                f"recipe_description_{recipe_name}",
                recipe_description,
                "Arial",
        ...
        if result_surf is None:
            result_base_id = (
                result_icon_id
                if self.renderer.getTexture(result_icon_id)
                else "placeholder-item"
            )
            result_surf = self.renderer.ResizeTexture(
                result_base_id, [result_icon_size, result_icon_size], False, save=False
            )
            if result_surf:
        ...
        if not can_craft:
            button_sprite_id = (
                self.button_normal_id
            )  # Normal state, will be tinted gray
            tint_disabled = True
            brightness_boost = 0
        elif is_hovering:
            button_sprite_id = (
                self.button_normal_id
            )  # Normal state with brightness boost for hover
            tint_disabled = False
        ...
        if button_tex:
            scaled_button = pygame.transform.scale(
                button_tex, (button_width, button_height)
            )
            if tint_disabled:
                ...
                scaled_button.blit(
                    gray_surface, (0, 0), special_flags=pygame.BLEND_RGBA_MULT
                )
            elif brightness_boost > 0:
                # Apply brightness boost for hover state
                bright_surface = pygame.Surface(
                    scaled_button.get_size(), pygame.SRCALPHA
                )
                bright_surface.fill(
                    (brightness_boost, brightness_boost, brightness_boost, 0)
                )
                scaled_button.blit(
                    bright_surface, (0, 0), special_flags=pygame.BLEND_RGBA_ADD
                )
            ...
            pygame.draw.rect(
                self.renderer.screen, (255, 255, 255), self.craft_button_rect, 2
            )
        ...
        self.renderer.createAndRenderText(
            ...
            result_font_size,
            text_color,
            (result_name_x, result_y + result_icon_size),
            cache=False,
            silence=True,
        )
...
class BlockManager:
    ...
    def place_block(self, player_x, direction):
        ...
        is_solid = self.world.blockProperties.get(block_type_normalized, {}).get(
            "solid", True
        )
        ...
        initial_state = (
            "idle"
            if block_type_normalized == "cooker"
            else ("broken" if block_type_normalized == "time machine" else "default")
        )
        ...
        self.placed_blocks.append(
            {
                ...
                "height": self.world.blockProperties.get(block_type_normalized, {}).get(
                    "height", 1
                ),  # Block height in blocks
                "state": initial_state,  # Initial state for different block types
            }
        )

        if (
            self.world.blockProperties.get(block_type_normalized, {}).get("height", 1)
            == 2
        ):
            self.placed_blocks.append(
                {
    ...
    def check_collision(self, player_x, player_y, direction):
        ...
        for block in self.placed_blocks:
            ...
            if player_bottom > block_top:
                if direction == 1:
                    if (
                        player_right >= block_left - 5
                        and player_right <= block_right
                        and player_left < block_left
                    ):
                        return True
                elif direction == -1:
                    if (
                        player_left <= block_right + 5
                        and player_left >= block_left
                        and player_right > block_right
                    ):
                        return True
                    ...
                    if block_type == "time machine":
                        ...
                        if surf:
                            self.renderer.render(
                                [surf],
                                [(pos_on_screen, self.groundy - self.block_size)],
                            )
                            ...
                            if block_state == "broken":
                                ...
                                instruction_x = (
                                    pos_on_screen + self.block_size // 2 - 80
                                )
                                instruction_y = self.groundy - self.block_size - 10
                                self.renderer.createAndRenderText(
                                    f"fix_tm_{block['x']}",
                                    "Use screwdriver to fix",
                                    "Arial",
                                    12,
                                    (255, 100, 100),
                        ...
                        if surf:
                            self.renderer.render(
                                [surf], [(pos_on_screen, self.groundy)]
                            )
                else:
...
class CookerManager:
    ...
    def recalcLayout(self):
        ...
        self.screen_w = w
        self.screen_h = h
        self.renderer.createSolidTexture(
            self.overlay_id, (0, 0, 0, 180), (w, h), 4, cache=True
        )
        gui_texture = self.renderer.getTexture(self.gui_base)
        if gui_texture:
            gui_w = int(w * 0.6)
        ...
        if self.gui_scaled:
            ...
            if self.is_smelting:
                ...
                self.renderer.createAndRenderText(
                    ...
                    14,
                    text_color,
                    (bar_x + (bar_width - prog_width) // 2, bar_y + 3),
                    cache=False,
                    silence=True,
                )
            ...
            for idx, recipe_name in enumerate(self.recipe_list):
                ...
                self.hotbarSprite.doSpriteAnimation(
                    1, slot_pos, (self.slot_size, self.slot_size)
                )
    ...
    def render_cooking_slots(self):
        ...
        # Draw fuel slot
        pygame.draw.rect(
            self.renderer.screen, (80, 60, 40), (fuel_x, top_y, slot_size, slot_size)
        )
        pygame.draw.rect(
            self.renderer.screen,
            (200, 150, 100),
        ...
        self.renderer.createAndRenderText(
            ...
            font_size,
            label_color,
            (ingredient_x + (slot_size - ing_width) // 2, top_y - 18),
            cache=False,
            silence=True,
        )
        ...
        self.renderer.createAndRenderText(
            ...
            font_size,
            label_color,
            (fuel_x + (slot_size - fuel_width) // 2, top_y - 18),
            cache=False,
            silence=True,
        )
        ...
        self.renderer.createAndRenderText(
            ...
            font_size,
            label_color,
            (output_x + (slot_size - output_width) // 2, output_y - 18),
            cache=False,
            silence=True,
        )
        ...
        if self.is_smelting:
            if self.current_input_id >= 0:
                ...
                if surf is None:
                    base_id = (
                        input_icon_id
                        if self.renderer.getTexture(input_icon_id)
                        else "placeholder-item"
                    )
                    surf = self.renderer.ResizeTexture(
                        base_id, [icon_size, icon_size], False, save=False
            ...
            if self.current_fuel_id >= 0:
                ...
                if surf is None:
                    base_id = (
                        fuel_icon_id
                        if self.renderer.getTexture(fuel_icon_id)
                        else "placeholder-item"
                    )
                    surf = self.renderer.ResizeTexture(
                        base_id, [icon_size, icon_size], False, save=False
            ...
            if self.current_output_id >= 0:
                ...
                if surf is None:
                    base_id = (
                        output_icon_id
                        if self.renderer.getTexture(output_icon_id)
                        else "placeholder-item"
                    )
                    surf = self.renderer.ResizeTexture(
                        base_id, [icon_size, icon_size], False, save=False
            ...
            self.hovered_slot >= 0
            and self.hovered_slot < len(self.recipe_list)
            and not self.is_smelting
        ):
            ...
            if recipe_data:
                ...
                self.renderer.createAndRenderText(
                    f"preview_input_amt_{self.hovered_slot}",
                    input_amt_text,
                    "arial",
                    font_size_small,
                    input_color,
                    (
                        ingredient_x + (slot_size - amt_width) // 2,
                        top_y + slot_size - 15,
                    ),
                    cache=False,
                    silence=True,
                ...
                self.renderer.createAndRenderText(
                    f"preview_fuel_amt_{self.hovered_slot}",
                    fuel_amt_text,
                    "arial",
                    font_size_small,
                    fuel_color,
                    (fuel_x + (slot_size - amt_width) // 2, top_y + slot_size - 15),
                    cache=False,
                    silence=True,
                )
                ...
                self.renderer.createAndRenderText(
                    f"preview_output_amt_{self.hovered_slot}",
                    output_amt_text,
                    "arial",
                    font_size_small,
                    output_color,
                    (
                        output_x + (slot_size - amt_width) // 2,
                        output_y + slot_size - 15,
                    ),
                    cache=False,
                    silence=True,
                )
