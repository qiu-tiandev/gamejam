...
class Ground:
    ...
    def renderGround(self, player):
        ...
        while x < screenx:
            ...
            cropped = self.texture.subsurface(
                pygame.Rect(src, 0, visible_width, self.height)
            )
    ...
    def spawnRandomItems(self):
        if random.random() < 0.00055:
            ...
            if mysterious_meat_id >= 0:
                ...
                spawn_x = self.player.x + random.randint(
    ...
    def pickUpItems(self):
        ...
        for i in self.items:
            ...
            if now - i["spawn"] < 2000:
                ...
                continue
            if (
                self.player.x + 30 > itemX
                and self.player.x < itemX + 30
                and self.player.y + 30 > self.groundy
                and self.player.y < self.groundy + 30
            ):
...
class ChestManager:
    def __init__(self, player, ground, world, id: str, itemManager: ItemEntityManager):
        ...
        self.drop_chances = {"Common": 1.0, "Uncommon": 0.5, "Rare": 0.3}
        self.currentId = 1
    ...
    def is_far_enough(self, pos: int) -> bool:
        for existing in self.chests.values():
            if abs(pos - existing) < self.minDist:
                return False
        if (
            self.lastChestPos is not None
            and abs(pos - self.lastChestPos) < self.minDist
        ):
            return False
        return True
        ...
        if pygame.key.get_pressed()[pygame.K_s]:
            for n, i in self.chests.items():
                if (
                    playerx + 60 > i
                    and playerx < i + 60
                    and playery + 60 > self.groundy
                    and playery < self.groundy + 60
                ):
                    for name, loot_data in self.loot.items():
                        ...
                        if random.random() < drop_chance:
                            ...
                            self.itemManager.addItemEntities(
                                item_id, amt, (drop_x, self.groundy)
                            )
                    del self.chests[n]
                    break
    ...
    def generateAndRenderChest(self):
        ...
        if (
            random.random() < self.spawnChance
            and time_since_last_spawn >= self.cooldown_duration
        ):
            chestPos = self.player.x + random.randint(
                self.screenx // 2 + 50, self.screenx // 2 + 300
            )
        ...
        if render_pos:
            self.renderer.render([self.id] * len(render_pos), render_pos)
...
class MonsterManager:
    def __init__(
        ...
        if world and world.world_index >= 1:
            self.loot_table["Half-eaten Brain"] = {
                "min": 1,
                "max": 1,
                "rarity": "Uncommon",
            }
        ...
        self.renderer.createSolidTexture(
            "health_bar_bg", (100, 100, 100), (self.monster_size, 6), 4, True
        )
        self.renderer.createSolidTexture(
            "health_bar_fg", (0, 255, 0), (self.monster_size, 6), 4, True
        )
    ...
    def spawnMonster(self):
        ...
        if self.block_manager:
            brain_blocks = [
                b
                for b in self.block_manager.placed_blocks
                if b.get("type") == "half-eaten brain"
            ]
            if brain_blocks:
        ...
        if random.random() < adjusted_spawn_chance:
            ...
            spawn_x = self.player.x + side * (
                self.screenx // 2 + random.randint(100, 300)
            )
            ...
            self.monsters.append(
                {
                    ...
                    "direction": (
                        1 if spawn_x > self.player.x else -1
    ...
    def updateMonsters(self, is_focused=True):
        ...
        for monster in self.monsters:
            ...
            if nearest_brain is not None and nearest_brain_dist < 2000:  # 2000px range
                ...
                is_in_contact = (
                    monster_right > brain_left - 20 and monster_left < brain_right + 20
                )
                ...
                if is_in_contact:
                    ...
                    if nearest_brain["contact_start"] is not None:
                        contact_duration = (
                            adjusted_time * 1000 - nearest_brain["contact_start"]
                        )
            ...
            if target_x > monster["x"]:
                ...
                direction = monster.get(
                    "direction", 1
                )  # Keep current direction if no movement
            ...
            if self.block_manager and not monster["jumping"]:
                ...
                for block in self.block_manager.placed_blocks:
                    ...
                    if direction == 1:  # Moving right
                        if (
                            block_left > monster_right
                            and block_left <= monster_right + 20
                        ):
                            should_jump = True
                            break
                    elif direction == -1:  # Moving left
                        if (
                            block_right < monster_left
                            and block_right >= monster_left - 20
                        ):
                            should_jump = True
                            break
            ...
            if (
                should_jump
                and not monster["jumping"]
                and monster["y"] >= ground_level - 5
            ):
            ...
            if self.block_manager and direction != 0:
                ...
                for block in self.block_manager.placed_blocks:
                    ...
                    if monster_bottom > block_top:
                        if direction == 1:  # Moving right
                            if (
                                monster_right >= block_left - 10
                                and monster_right <= block_right + 5
                                and monster["x"] < block_left
                            ):
                                can_move = False
                                break
                        elif direction == -1:  # Moving left
                            if (
                                monster_left <= block_right + 10
                                and monster_left >= block_left - 5
                                and monster["x"] > block_right
                            ):
                                can_move = False
                                break
            ...
            if (
                should_despawn
                and monster.get("health", 100) <= 0
                and not monster.get("loot_dropped", False)
            ):
        ...
        if self.block_manager and brains_to_remove:
            self.block_manager.placed_blocks = [
                b for b in self.block_manager.placed_blocks if b not in brains_to_remove
            ]
    ...
    def renderMonsters(self):
        ...
        ) % 20  # Change frame every 20 ticks
        ...
        for monster in self.monsters:
            ...
            if -100 <= pos_on_screen <= screenx + 100:
                ...
                if zombie_tex:
                    resized_zombie = self.renderer.ResizeTexture(
                        zombie_frame_id,
                    ...
                    if resized_zombie:
                        ...
                        if direction < 0:  # Facing left
                            flipped_zombie = pygame.transform.flip(
                                resized_zombie, True, False
                            )
                            self.renderer.render_surface(
                                flipped_zombie,
                                (int(monster_screen_x), int(monster_screen_y)),
                            )
                        else:  # Facing right
                            self.renderer.render_surface(
                                resized_zombie,
                ...
                health_percent = monster.get("health", 100) / monster.get(
                    "max_health", 100
                )
                ...
                self.renderer.render(
                    ["health_bar_bg"], [(monster_screen_x, health_bar_y)]
                )
                ...
                if health_percent > 0:
                    ...
                    self.renderer.render_surface(
                        health_surface, (monster_screen_x, health_bar_y)
                    )
    ...
    def checkLaserCollision(self, lasers):
        ...
        for laser_idx, laser in enumerate(lasers):
            ...
            for monster in self.monsters:
                ...
                monster_screen_x = (
                    monster["x"] - player_x + screenx // 2 - hitbox_size // 2
                )
                monster_screen_y = (
                    monster["y"] - (hitbox_size - self.monster_size) // 2
                )  # Adjust y to center hitbox
                ...
                if (
                    laser_right > monster_screen_x
                    and laser_x < monster_right
                    and laser_bottom > monster_screen_y
                    and laser_y < monster_bottom
                ):
                    # Hit! Reduce health with player's damage boost applied
                    base_damage = random.randint(10, 25)
                    total_boost = (
                        self.player.hunger_damage_boost + self.player.damage_boost
                    )
    ...
    def checkPlayerCollision(self):
        ...
        for monster in self.monsters:
            ...
            if (
                player_x < monster["x"] + monster_width
                and player_x + player_width > monster["x"]
                and player_y < monster["y"] + monster_height
                and player_y + player_height > monster["y"]
            ):
...
class BeaconManager:
    ...
    def update(self, dt, is_paused=False):
        ...
        beacon_blocks = [
            b for b in self.block_manager.placed_blocks if b.get("type") == "beacon"
        ]
        ...
        for beacon in beacon_blocks:
            ...
            if beacon_id not in self.beacons:
                self.beacons[beacon_id] = {"laser_cooldown": 0, "active_lasers": []}

        # Remove beacons that no longer exist
        existing_ids = {id(b) for b in beacon_blocks}
        self.beacons = {
            bid: state for bid, state in self.beacons.items() if bid in existing_ids
        }
        ...
        for beacon in beacon_blocks:
            ...
            if beacon_state["laser_cooldown"] <= 0 and self.monster_manager.monsters:
                ...
                beacon_center_y = (
                    self.block_manager.groundy + 10
                )  # Shoot from a bit lower
                ...
                for monster in self.monster_manager.monsters:
                    ...
                    dy = (
                        monster["y"]
                        - self.monster_manager.monster_size // 4
                        - beacon_center_y
                    )
                    dist = (dx**2 + dy**2) ** 0.5
                ...
                if nearest_monster and nearest_dist <= self.beacon_range:
                    ...
                    target_y = (
                        nearest_monster["y"] - self.monster_manager.monster_size // 4
                    )  # Aim at monster middle/top
        ...
        for beacon_state in self.beacons.values():
            for laser in beacon_state["active_lasers"]:
                ...
                self.renderer.createAndRenderSolidTexture(
                    ...
                    (255, 0, 0),
                    (10, 10),
                    (int(laser_screen_x), int(laser_screen_y)),
                    4,
                    cache=False,
                )
        ...
        for beacon_state in self.beacons.values():
            ...
            for laser_idx, laser in enumerate(beacon_state["active_lasers"]):
                ...
                for monster in self.monster_manager.monsters:
                    ...
                    if (
                        laser_x >= monster_x
                        and laser_x <= monster_x + monster_size
                        and laser_y >= monster_y
                        and laser_y <= monster_y + monster_size
                    ):
                        # Hit! Reduce health
                        base_damage = random.randint(10, 25)
                        total_boost = (
