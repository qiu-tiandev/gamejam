...
class Player:
    def __init__(self, world, ground):
        ...
        self.hunger_timer = (
            self.max_hunger_time
        )  # Countdown from max_hunger_time seconds
        self.last_food_time = pygame.time.get_ticks() / 1000  # Last time player ate
        self.hunger_damage_rate = (
            self.max_hunger_time / 4
        )  # Damage scales with hunger time
    ...
    def detectinput(self):
        ...
        if keys:
            for key, direction in self.directions.items():
                if keys[key]:
                    if self.block_manager and self.block_manager.check_collision(
                        self.x, self.y, direction[0]
                    ):
                        continue
            ...
            if keys[pygame.K_SPACE]:
                on_block = (
                    self.block_manager
                    and self.block_manager.get_block_under_player(self.x, self.y)
                )
                if not self.jumping and (
                    self.ground.isTouchingGround(self.y) or on_block
                ):
                    self.jumping = True
    ...
    def move(self, is_paused=False):
        ...
        on_block = self.block_manager and self.block_manager.get_block_under_player(
            self.x, self.y
        )
    ...
    def _load_player_sprites(self):
        ...
        for i in range(1, 12):
            ...
            if self.renderer.getTexture(frame_id):
                self.renderer.ResizeTexture(
                    frame_id, [scaled_width, 50], False, save=True
                )
    ...
    def _update_animation(self):
        ...
        if self.jumping:
            ...
            if elapsed < self.jump_duration * 1000 * 0.5:  # First half - ascending
                ...
                frame_index = min(
                    int((elapsed / (self.jump_duration * 500)) * len(frames)),
                    len(frames) - 1,
                )
                ...
                frame_index = min(
                    int((desc_time / (self.peak_jump_duration * 1000)) * len(frames)),
                    len(frames) - 1,
                )
                self.current_frame = frames[frame_index]
        elif self.is_moving:
            ...
            if self.animation_timer >= self.animation_speed:
                ...
                frame_idx = (
                    (frames.index(self.current_frame) + 1) % len(frames)
    ...
    def take_damage(self, amount=None):
        ...
        if amount is None:
            amount = random.randint(
                self.monster_hit_damage[0], self.monster_hit_damage[1]
            )
    ...
    def update_health(self, is_paused=False):
        ...
        if adjusted_time - self.last_hit_time >= self.regen_delay:
            ...
            if hunger_percent > 75:
                ...
                current_regen_interval = (
                    self.regen_interval * 0.3
                )  # 3x faster (0.9 seconds instead of 3)
            else:
    ...
    def update_hunger(self, is_paused=False):
        ...
        self.hunger_timer = max(
            0, min(self.max_hunger_time, self.max_hunger_time - time_since_food)
        )
