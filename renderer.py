...
import math
class Renderer:
    def __init__(self, Imagetextures: dict, TextTextures:dict[str, tuple[str,str,int,str]]): #enforce pre-rendering of textures
        self.Imagetextures = {}
        self.TextTextures = {}
        self.visibility = {}
        self.fonts = {}
    ...
    def createAndRenderText(self, screen,id:str, text:str,font:str, font_size:int, color:tuple[int,int,int], coordinates:tuple[int,int],cache:bool=False):
        texture = self.createText(id, text,font, font_size, color, cache)
        screen.blit(texture, coordinates)
    def createAndRenderImage(self,screen,id:str, filepath:str,size:tuple[int,int], coordinates:tuple[int,int],cache:bool=False):
