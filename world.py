...
class Sky:
    ...
    def render(self, player_x):
        ...
        if self.scaled_w > self.screen_w:
            crop_rect = pygame.Rect(offset_x, 0, min(self.screen_w, self.scaled_w - offset_x), self.renderer.getTexture(self.scaled_id).get_height())
            cropped = self.renderer.getTexture(self.scaled_id).subsurface(crop_rect)
            self.renderer.render([cropped], [(0, 0)])
            if crop_rect.width < self.screen_w:
                remaining = self.screen_w - crop_rect.width
                wrap_rect = pygame.Rect(0, 0, remaining, crop_rect.height)
                wrapped = self.renderer.getTexture(self.scaled_id).subsurface(wrap_rect)
                self.renderer.render([wrapped], [(crop_rect.width, 0)])
        else:
