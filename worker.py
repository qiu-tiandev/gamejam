...
util.setRenderer(
    Renderer(
        screen,
        {
            ...
            "time_machine_block": "time_machine.png",
            "player_spritesheet": "player.png",
        },
        {"crafting_title": ("Crafting", "arial", 24, (255, 255, 255))},
    )
)
...
cooker_sprite.save_frames(
    "block_cooker"
)  # Creates block_cooker_001 (idle) and block_cooker_002 (cooking)
...
def render_intro_sequence():
    ...
    if black_sky and (
        black_sky.get_width() != current_w or black_sky.get_height() != current_h
    ):
        renderer.createImage("black_sky", "black_sky.png", (current_w, current_h))

    globe_tex = renderer.getTexture("globe")
    if globe_tex and (
        globe_tex.get_width() != globe_size or globe_tex.get_height() != globe_size
    ):
        renderer.createImage("globe", "globe.png", (globe_size, globe_size))

    spaceship_tex = renderer.getTexture("spaceship")
    if spaceship_tex and (
        spaceship_tex.get_width() != spaceship_size
        or spaceship_tex.get_height() != spaceship_size
    ):
        renderer.createImage(
            "spaceship", "spaceship.png", (spaceship_size, spaceship_size)
        )
...
def create_world_entities(world_index):
    ...
    world_recipes = {
        ...
        4: {  # #AWRZ-P
            ...
            "time machine": [
                ("Magical Arbitrary Screw", 2),
                ("metal scrap", 5),
                ("Corrupted Fragment", 2),
                ("mysterious powder", 5),
                ("battery", 10),
                ("screwdriver", 1),
            ],
        ...
        5: {  # Blackhole
            ...
            "time machine": [
                ("Magical Dark-matter Screw", 2),
                ("metal scrap", 5),
                ("Blackhole Fragment", 2),
                ("mysterious powder", 5),
                ("battery", 10),
                ("screwdriver", 1),
            ],
        ...
        6: {  # Whitehole
            ...
            "time machine": [
                ("Magical Anti-matter Screw", 2),
                ("metal scrap", 5),
                ("Whitehole Fragment", 2),
                ("mysterious powder", 5),
                ("battery", 10),
                ("screwdriver", 1),
            ],
    ...
    if world_index >= 1:  # Limbo
        world_cooking_recipes["recipes"].update(
            {"Chalk": [(1, "Calcium", 1)], "Ash": [(1, "bones", 1)]}
        )
    ...
    world = World(
        ...
        chestLoot=world_chest_loot,
        craftingRecipes=world_crafting_recipes,
        cookingRecipes=world_cooking_recipes,
    )
...
def initialize_managers(world, ground, player, inventory):
    ...
    monster_manager = entities.MonsterManager(
        player, ground, block_manager, itemManager=item_manager, world=world
    )
    cooker_manager = CookerManager(world, inventory, player, ground, block_manager)
    beacon_manager = entities.BeaconManager(
        player, ground, block_manager, monster_manager
    )
...
def render_unlock_screen():
    ...
    start_x = (
        gui_x
        + gui_padding
        + (gui_width - gui_padding * 2 - total_items_width) // 2
        + item_size // 2
    )
    ...
    for i, item_id in enumerate(unlocked_items):
        ...
        surf = renderer.ResizeTexture(
            icon_id, [item_size, item_size], False, save=False
        )
        if surf:
            renderer.render(
                [surf], [(icon_x - item_size // 2, icon_y - item_size // 2)]
            )
...
def render_death_screen():
    ...
    renderer.createAndRenderText(
        ...
        14,
        text_color,
        (gui_x + (gui_width - text1_width) // 2, gui_y + 70),
        cache=False,
        silence=True,
    )
    renderer.createAndRenderText(
        ...
        14,
        text_color,
        (gui_x + (gui_width - text2_width) // 2, gui_y + 90),
        cache=False,
        silence=True,
    )
    renderer.createAndRenderText(
        ...
        14,
        text_color,
        (gui_x + (gui_width - text3_width) // 2, gui_y + 110),
        cache=False,
        silence=True,
    )
    ...
    death_respawn_button_rect = pygame.Rect(
...
def render_win_screen():
    ...
    renderer.createAndRenderText(
        ...
        48,
        text_color,
        (screen_w // 2 - text_width // 2, screen_h // 2 - 24),
        cache=False,
        silence=True,
    )
...
(
    ...
    cookerManager,
    beaconManager,
) = initialize_managers(current_world, ground, player, inventory)
...
while not time_machine_used:
    ...
    if current_w != w or current_h != h:
        ...
        renderer.createSolidTexture(
            craftManager.overlay_id, (0, 0, 0, 180), (w, h), 4, cache=True
        )
    ...
    if grace_period_active and pygame.time.get_ticks() >= grace_period_end_time:
        grace_period_active = False
        player.last_food_time = (
            pygame.time.get_ticks() // 1000
        )  # Reset hunger timer after grace period
    ...
    if not is_paused:  # Only update game logic when not paused
        ...
        if (
            not grace_period_active
            and not is_paused
            and not tutorialManager.is_active
            and not death_screen_active
        ):
        ...
        renderer.createAndRenderText(
            "OBJECTIVE",
            "Objective: Get time machine to progress",
            "Arial",
            16,
            text_color,
            (10, 0),
        ...
        if items_list and inventory.currentSlot - 1 < len(items_list):
            ...
            if amount > 0:
                ...
                if item_name.lower() in current_world.placeableBlocks:
                    ...
                    has_battery = (
                        battery_id != -1 and inventory.items.get(battery_id, 0) > 0
                    )
    ...
    if item_description and not tutorialManager.is_active:
        ...
        renderer.createAndRenderText(
            ...
            18,
            text_color,
            (w // 2 - len(item_description) * 4, description_y),
            cache=False,
            silence=True,
        )
    ...
    if keybind_guide and not tutorialManager.is_active:
        renderer.createAndRenderText(
            ...
            18,
            text_color,
            (w // 2 - len(keybind_guide) * 4, guide_y),
            cache=False,
            silence=True,
        )
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:  # Left mouse button
                ...
                if tutorialManager.is_active:
                    ...
                    if hasattr(
                        craftManager, "craft_button_rect"
                    ) and craftManager.craft_button_rect.collidepoint(mouse_pos):
        ...
        ):  # sum keys for input (exclude a and d for player movement)
            ...
            if pygame.K_1 <= event.key <= pygame.K_9:
                ...
                if items_list and inventory.currentSlot - 1 < len(items_list):
                    ...
                    if item_name == "Clock" and amount > 0:
                        ...
                        if (
                            nearby_time_machine
                            and nearby_time_machine.get("state") == "broken"
                        ):
                ...
                # Check if near a fixed time machine (time travel!)
                    ...
                    if (
                        nearby_time_machine
                        and nearby_time_machine.get("state") == "fixed"
                    ):
                        ...
                        ground, sky, current_world = create_world_entities(
                            current_world_index
                        )
                        ...
                        grace_period_end_time = (
                            pygame.time.get_ticks() + GRACE_PERIOD_DURATION
                        )
                        ...
                        console.sendInfo(
                            f"Time traveled to {current_world.world_name}!", __file__
                        )
                    else:
                        ...
                        for n, chest_x in list(chestManager.chests.items()):
                            if (
                                player.x + 60 > chest_x
                                and player.x < chest_x + 60
                                and player.y + 60 > chestManager.groundy
                                and player.y < chestManager.groundy + 60
                            ):
                                for name, loot_data in chestManager.loot.items():
                                    rarity = loot_data["rarity"]
                                    drop_chance = chestManager.drop_chances.get(
                                        rarity, 1.0
                                    )
                                    if random.random() < drop_chance:
                                        ...
                                        itemManager.addItemEntities(
                                            item_id, amt, (drop_x, chestManager.groundy)
                                        )
